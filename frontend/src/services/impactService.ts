...
function mapView(view: ImpactView, detail?: NewsDetail): ImpactEvent {
  ...
  return {
    ...
    impactDirection: primary?.direction === 'POSITIVE' ? 'BULLISH' : primary?.direction === 'NEGATIVE' ? 'BEARISH' : 'NEUTRAL', priceChange: primary?.priceChangePct ?? 0, portfolioImpact: asNumber(primary?.valueImpact),
...
export const impactService = {
  async getImpactEvents(portfolioId: number, symbol?: string) {
    ...
    // Fetch remaining pages so recently-scored articles are not hidden behind
    // older headlines on later pages (API returns newest first).
    const allRows = [...news.content]
    for (let p = 2; p <= news.totalPages; p++) {
      const nextQuery = new URLSearchParams({ page: String(p), size: '100' })
      if (symbol) nextQuery.set('symbol', symbol)
      try {
        const nextPage = await apiFetch<PagedResponse<NewsRow>>(`/news?${nextQuery}`)
        allRows.push(...nextPage.content)
      } catch { break }
    }
    const results = await Promise.all(allRows.filter((row) => row.hasImpact).map(async (row) => {
